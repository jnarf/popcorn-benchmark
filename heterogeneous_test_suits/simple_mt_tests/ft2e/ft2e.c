...
/*
 * ft2e - Simple Migration Functional 2 Node Test D
 *
 ...
 * Threads will modify a global and local counter. The purpose is to have a simple test of the page coherency protocol.
...
{
	int current_nid;
	struct popcorn_node_status pnodes[FT2E_NODES];
	int node_err = 0;

	/* Get Node Info. Make sure We can retrieve the right node's information */
    node_err = popcorn_getnodeinfo(&current_nid, pnodes); //FT2E_NODES);

	if (node_err) {
		printf("FT_2_E FAILED: popcorn_get_node_info, Cannot retrieve the nodes' information at node %d. ERROR CODE %d\n", current_nid, node_err);
		return node_err;
	}

	/* Testing Node Info */
	if(current_nid != local_nid) {
		printf("FT_2_E FAILED: We should be at Node %d. Yet we are at node %d\n", local_nid, current_nid);
		node_err = -1;
		return node_err;
	} else if(pnodes[local_nid].status == NODE_OFFLINE) {
		printf("FT_2_E FAILED: Node %d is offline.\n", local_nid);
		node_err = -1;
		return node_err;
		...
		printf("FT_2_E WARN: Node %d does not have expected X86 architecture. Architecture is %s.\n", local_nid, arch_sz[pnodes[local_nid].arch + 1]);
...
int thread_sanity_check(int nid, pid_t tid)
{
	...
	if (thread_err) {
		printf("FT_2_E FAILED: popcorn_get_status, Cannot retrieve the thread' information at node %d. ERROR CODE: %d\n", nid, thread_err);
		return thread_err;
	}

	if(status.current_nid != nid) {
		printf("FT_2_E FAILED: popcorn_get_status, Thread %d should be at node %d. But instead it is at node %d\n", tid, nid, status.current_nid);
		thread_err = -1;
		return thread_err;
	}
...
static void *child_thread(void *input)
{
	...
	if(source_tid == -1) {
		printf("FT_2_E FAILED: Thread ID is not a positive integer, TID: %d\n", source_tid);
	...
	if(current_thread->err) {
		switch (current_thread->err) {
			case -EINVAL:
				printf("FT_2_E FAILED: Thread %d. Invalid Migration Destination %d\n", source_tid, current_thread->sink_nid);
				current_thread->done = 1;
				break;
			case -EBUSY:
				printf("FT_2_E FAILED: Thread %d already running at destination %d\n", source_tid, current_thread->sink_nid);
				current_thread->done = 1;
				break;
			case -EAGAIN:
				printf("FT_2_E FAILED: Thread %d could not reach destination %d. Node is offline.\n", source_tid, current_thread->sink_nid);
				current_thread->done = 1;
				break;
			default:
				printf("FT_2_E FAILED: Thread %d could not migrate, process_server_do_migration returned %d\n", source_tid, current_thread->err);
				current_thread->done = 1;
		}
	}
	...
	//printf("FT_2_E: We should have arrived at sink node.\n");
	...
	/* mark local thread migration counter */
	tcount++;

	/* Migrate Thread Back to source_nid */
	current_thread->err = migrate(current_thread->source_nid, NULL, NULL);

	printf("FT_2_E[%d]: We should have arrived back at source node.\n", source_tid);

	printf("FT_2_E[%d]: Global counter %d Thread counter %d\n", source_tid, gcount, tcount);

	/* We should be at sink_nid. Get TID at Sink Node */
	temp_tid = popcorn_gettid();

	if(temp_tid == -1) {
		printf("FT_2_E[%d] FAILED: Thread ID is not a positive integer, TID: %d\n", source_tid, temp_tid);
		current_thread->err = -1;
		current_thread->done = 1;
		all_done++;
		return NULL;
	} else if(temp_tid != source_tid) {
		printf("FT_2_E[%d] FAILED: Thread ID %d does not match original TID %d\n", source_tid, temp_tid, source_tid);
		current_thread->err = -1;
		current_thread->done = 1;
		return NULL;
	}
...
int main(int argc, char *argv[])
{
    ...
    if(source_pid == -1) {
        printf("FT_2_E FAILED: Process ID is not a positive integer, PID: %d\n", source_pid); // NOTE TID (TGID) should be == to PID (TID) in this instance
        ft2e_errno = -1;
        return ft2e_errno;
    }

	if (argc != 4) {
        printf("FT_2_E FAILED: This test takes 3 arguments, Source Node ID, Sink Node ID, # of threads.\n");
    }


	printf("FT_2_E: Process ID is %d\n", source_pid);
    ...
    if(source_node == sink_node) {
        printf("FT_2_E FAILED: Source Node ID must be different to Sink Node ID\n");
        ft2e_errno = -1;
        return ft2e_errno;
    } else if((source_node | sink_node) < 0 || source_node >= MAX_POPCORN_NODES || sink_node >= MAX_POPCORN_NODES) {
        printf("FT_2_E FAILED: Node ID's must be a positive integer 0-31\n");
        ft2e_errno = -1;
        return ft2e_errno;
    }

    /* Init popcorn "traveling" threads */
    if((ft2e_errno = __init_thread_params())){
        printf("FT_2_E FAILED: __init_thread_params() failed error %d\n", ft2e_errno);
        return ft2e_errno;
    }
    for(i = 0; i < nthreads; i++) {
    ...
    for(i = 0; i < nthreads; i++) {
	int *errno_ptr = &ft2e_errno;
	pcn_thread = threads[i];
    	pthread_join(pcn_thread->thread_info, (void(**))(&errno_ptr));
    	printf("FT_2_E TEST at NODE %d Thread %d exited with CODE %d\n", source_node, pcn_thread->tid, pcn_thread->err);
    	free(pcn_thread);
    }

    printf("FT_2_E TEST PASSED at NODE %d\n", source_node);
    return ft2e_errno;
#else
	int ft2e_errno;
	printf("FT_2_E: Test only supports X86_64 Architecture\n");
	ft2e_errno = -1;
	return ft2e_errno;
#endif
}
