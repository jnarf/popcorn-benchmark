...
/* POPCORN LIBRARY DECLARATIONS AND DEFINITIONS LOCATED AT /USR/LOCADL/POPCORN/X86_64/INCLUDE */
#include <platform.h>
...
int main(int argc, char *argv[]) {
...
#ifdef __x86_64__
    struct popcorn_node_status pnodes[FT2A_NODES];
    ...
    f2ta_errno = popcorn_getnodeinfo(&current_nid, pnodes);// FT2A_NODES);
    ...
    printf("FT_2_A INFO: Node %d architecture is %s.\n", source_node, arch_sz[pnodes[source_node].arch + 1]);

    printf("FT_2_A INFO: Node %d architecture is %s.\n", sink_node, arch_sz[pnodes[sink_node].arch + 1]);


    /*Get process status. */
    f2ta_errno = popcorn_getthreadinfo(&status);
